...
public interface BoardRepository extends JpaRepository<Board, Long> {
    ...
    @Query(value = "SELECT b FROM Board AS b JOIN FETCH b.boardBody " +
            "JOIN FETCH b.category JOIN FETCH b.member JOIN FETCH b.files " +
            "where b.id = :id")
    Board findBoardById(@Param("id") Long id);

    // 모든 게시글 불러오기
    @Query(value = "SELECT new com.examples.crazy.simpleboard.dto.BoardsDto " +
            "(b.id, b.title, m.alias, b.readCount, b.createdDate) " +
            "FROM Board AS b INNER JOIN Member AS m ON b.member.id = m.id " +
