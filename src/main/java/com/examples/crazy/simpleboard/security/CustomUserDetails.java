...
public class CustomUserDetails extends User {
    ...
    // `UserDetails`를 직접 구현하지 않고, `UserDetails`를 구현하고 있는 `org.springframework.security.core.userdetails.User`를 상속받아 구현
    // `UserDetails`는 id(`loginId`로 변경), password, 권한 정보에 대한 기능을 정의
    // 이러한 `UserDetails`를 구현하면서 추가적으로 loginId, `User`의 ID에 해당하는 값 등을 저장하도록 `CustomUserDetails` 객체를 생성
